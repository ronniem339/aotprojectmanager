...
window.MetadataTask = ({ video, settings, onUpdateTask, isLocked }) => {
    ...
    // State for chapters and drag-and-drop
    ...
    useEffect(() => {
        ...
        if (metadata) {
            ...
            const metaChapters = (metadata.chapters || []).map((chap, i) => ({...chap, id: `chap-${i}-${Date.now()}`}));
            const videoChapters = (video.chapters || []).map((chap, i) => ({...chap, id: `chap-${i}-${Date.now()}`}));
            setChapters(videoChapters.length > 0 ? videoChapters : metaChapters);
        }
...
Return a JSON object with one key: {"newDescription": "..."}`;
    ...
    const handleDeleteChapter = (indexToDelete) => {
        setChapters(chapters.filter((_, index) => index !== indexToDelete));
    };
    ...
    const handleDragEnd = (e) => {
        const list = e.currentTarget.closest('.chapter-list');
        if(list){
             Array.from(list.children).forEach(item => item.classList.remove('drag-over-indicator'));
        }
    }
    ...
    if (!video.tasks?.chaptersFinalized) {
        return (
            <div className="space-y-4">
                 <h3 className="text-lg font-semibold text-amber-400">Step 3: Edit Chapters</h3>
                        ...
                        >
                            <span className="cursor-grab text-gray-500 p-1" title="Drag to reorder">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                                  <path d="M2 10a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm3 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm3 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm3 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm3 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2zM2 6a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm3 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm3 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm3 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2zm3 0a1 1 0 1 1 0-2 1 1 0 0 1 0 2z"/>
                                </svg>
                            </span>
                            ...
                            <button onClick={() => handleDeleteChapter(i)} className="p-2 text-red-400 hover:text-red-300 rounded-full flex-shrink-0" title="Delete Chapter">
