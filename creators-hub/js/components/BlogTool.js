            ...
            <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
                <div className="glass-card p-6 rounded-lg w-full max-w-lg">
                    ...
                    <div className="flex justify-end space-x-4 mt-6">
                        <button onClick={() => setModalView(null)} className="btn-secondary">Cancel</button>
                        <button onClick={handleSubmit} disabled={!selectedVideoId || isGenerating} className="btn-primary">Generate Post</button>
                    </div>
                </div>
            </div>
        );
             ...
             <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
                <div className="glass-card p-6 rounded-lg w-full max-w-lg">
                    ...
                    <div className="space-y-4">
                        ...
                        <textarea value={specifics} onChange={e => setSpecifics(e.target.value)} className="form-textarea w-full" rows="3" placeholder="Any other specific details? (e.g., focus on budget hotels, include historical sites)"></textarea>
                    </div>
                    <div className="flex justify-end space-x-4 mt-6">
                        <button onClick={() => setModalView(null)} className="btn-secondary">Cancel</button>
                        <button onClick={handleSubmit} disabled={!location || isGenerating} className="btn-primary">Generate Post</button>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="fixed inset-0 bg-black bg-opacity-70 flex justify-center items-center z-50">
                <div className="glass-card p-6 rounded-lg w-full max-w-xl max-h-[90vh] flex flex-col">
                    ...
                    <div className="flex gap-2 mb-4">
                        ...
                        <button onClick={fetchArticles} disabled={!location || isFetching} className="btn-secondary">
                            {isFetching ? 'Fetching...' : 'Find Existing Articles'}
                        </button>
                    </div>
                    <div className="flex-grow overflow-y-auto space-y-2 pr-2">
                        {isFetching && <LoadingSpinner text="Searching for articles..." />}
                     ...
                     <div className="flex justify-end space-x-4 mt-6 pt-4 border-t border-gray-700">
                        <button onClick={() => setModalView(null)} className="btn-secondary">Cancel</button>
                        <button onClick={handleSubmit} disabled={selectedArticles.size === 0 || isGenerating} className="btn-primary">Generate Guide</button>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="p-4 sm:p-6 md:p-8">
            <header className="flex flex-col sm:flex-row justify-between items-center mb-6 sm:mb-8 gap-4">
                ...
                <button onClick={onBack} className="btn-outline w-full sm:w-auto justify-center">
