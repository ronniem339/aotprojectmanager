...
(() => {
    ...
    window.OneOffWordPressUpdater = ({ appState }) => {
        ...
        const renderStatusBox = () => {
            if (isComplete) {
                return e('div', { className: 'text-center p-4 mt-4 bg-gray-900 rounded-lg' },
            ...
            if (status || error) {
                return e('div', { className: 'mt-4 p-4 rounded-lg bg-gray-900' },
                    e('p', { key: 'stat', className: 'font-mono text-sm text-white whitespace-pre-wrap' }, `${isRunning ? 'PROGRESS: ' : 'STATUS: '}${status}`),
                    error && e('p', { key: 'err', className: 'font-mono text-sm text-red-400 mt-2 whitespace-pre-wrap' }, `ERROR: ${error}`)
                );
            }
            return null;
        };
        ...
        return e('div', { className: 'border-t border-dashed border-blue-500 pt-10 mt-10 max-w-2xl' },
            ...
            e('div', { className: 'space-y-4 bg-gray-800/60 p-6 rounded-lg' },
                ...
                !isComplete && e('button', { onClick: handleRunUpdate, disabled: isRunning, className: 'btn btn-primary w-full' }, isRunning ? 'Updating...' : 'Fetch Tags & Update Posts')
            ),
            renderStatusBox()
        );
    };
